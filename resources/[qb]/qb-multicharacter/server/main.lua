...
QBCore.Functions.CreateCallback("qb-multicharacter:server:setupCharacters", function(source, cb)
    ...
    MySQL.query('SELECT * FROM players WHERE license = ?', {license}, function(result)
        for i = 1, (#result), 1 do
            ...
            result[i].inventory = json.decode(result[i].inventory)
            for j, item in ipairs(result[i].inventory) do
            ...
            result[i].cash = cashItem ~= nil and cashItem.amount or 0
