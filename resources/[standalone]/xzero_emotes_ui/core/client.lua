...
exportDP = exports['dpemotes']:getDP()
exportsDP = exports['dpemotes']
_dpEmoInsObj = nil
_emoItemList = {}
__Init = function()
    Wait(500)
...
_Events_Init = function()
    ...
    RegisterNUICallback(
        ...
        function(d)
            ...
            if d.etype then
                exportsDP:EmoteMenuStart(d.name, d.etype)
                return
            end
            if d.type == "Walks" then
                exportsDP:WalkMenuStart(d.name)
                return
            end
            if d.type == "Shared" then
                ...
                local b = d.typeby
                local c = d.label
                if a == nil or _DP.Shared[a] == nil and _DP.Dances[a] == nil then
                    return
                end
                local d, e = exportsDP:GetClosestPlayer()
                if e ~= -1 and e < 3 then
                    TriggerServerEvent("ServerEmoteRequest", GetPlayerServerId(d), a, b or nil)
                    exportsDP:SimpleNotify("Animacao com" .. GetPlayerName(d) .. " ~w~(~g~" .. c .. "~w~)")
                else
                    exportsDP:SimpleNotify("Ninguem por perto")
                end
                return
            end
        end
